...
const PhotoBooth = ({ setCapturedImages }) => {
  ...
  const capturePhoto = () => {
    ...
    if (video && canvas) {
        ...
        stickers.forEach(sticker => {
            ...
            context.drawImage(sticker, rect.left, rect.top, rect.width, rect.height);
        });
  ...
  const addSticker = (src) => {
    ...
    document.getElementById('camera').appendChild(sticker);
    ...
    sticker.addEventListener('drag', (e) => {
        sticker.style.left = e.clientX - sticker.width / 2 + 'px';
        sticker.style.top = e.clientY - sticker.height / 2 + 'px';
    });
  }
  ...
  return (
    <div className="photo-booth">
      ...
      <div className="photo-container">
        <div id="camera" className="camera-container">
      ...
      <div className="filters">
        ...
        <button onClick={() => setFilter("invert(100%)")} disabled={capturing}>Invert</button>
        <button onClick={() => setFilter("contrast(200%)")} disabled={capturing}>High Contrast</button>
      </div>

      <div className="stickers">
        <button onClick={() => addSticker('/love.png')}>Love</button>
        <button onClick={() => addSticker('/heart-shape.png')}>Sticker 2</button>
        <button onClick={() => addSticker('https://example.com/sticker3.png')}>Sticker 3</button>
      </div>
